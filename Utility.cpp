...
void useDefensiveItem(Character* character, Item& item)
{
    ...
    if( auto* currentChar = dynamic_cast<Dwarf*>(character) )
    {
        currentChar->boostArmor( item.getBoost() * 1.1 );
    }
    else if( auto* currentChar2 = dynamic_cast<Paladin*>(character) )
    {
        //same with paladins
        currentChar2->boostArmor( item.getBoost() * 1.3 );
    }
    else if( auto* currentChar3 = dynamic_cast<DragonSlayer*>(character))
    {
        currentChar3->boostArmor( item.getBoost() * 1.5 );
    }
    else if( auto* currentChar4 = dynamic_cast<Dragon*>(character) )
    {
...
void useHelpfulItem(Character* character, Item* item)
{
    if( auto* currentChar = dynamic_cast<Dwarf*>(character) )
    {
        currentChar->boostHitPoints(item->getBoost() * 2);
    }
    else if( auto* currentChar2 = dynamic_cast<Paladin*>(character) )
    {
        currentChar2->boostHitPoints(item->getBoost() * 1.5);
    }
    else if( auto* currentChar3 = dynamic_cast<DragonSlayer*>(character))
    {
        currentChar3->boostHitPoints(item->getBoost() * 1.25);
    }
    else if( auto* currentChar4 = dynamic_cast<Dragon*>(character) )
    {
...
void useAttackItem(Character* character, Item* item)
{
    if( auto* currentChar = dynamic_cast<Dwarf*>(character) )
    {
        currentChar->boostAttackDamage(item->getBoost() * 1.5);
    }
    else if( auto* currentChar2 = dynamic_cast<Paladin*>(character) )
    {
        currentChar2->boostAttackDamage(item->getBoost() * 1.33);
    }
    else if( auto* currentChar3 = dynamic_cast<DragonSlayer*>(character))
    {
        int currentAttackDamage = currentChar3->getAttackDamage();
        currentAttackDamage *= 10;
        currentChar3->boostAttackDamage(currentAttackDamage);
